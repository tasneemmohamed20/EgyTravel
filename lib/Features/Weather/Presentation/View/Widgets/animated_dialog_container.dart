...
import 'package:egy_travel/Features/Weather/Presentation/View/Widgets/a_i_weather_feedback.dart';
import 'package:egy_travel/Features/Weather/Presentation/View/Widgets/refresh_feed_back_button.dart';
...
class _AnimatedMetallicContainerState extends State<AnimatedMetallicContainer>
  ...
  final model = GenerativeModel(
    model: kGeminiModel,
    apiKey: kWeatherGeminiModelAPIKey,
  ); // Generative AI model
  ...
  Future<void> generateWeatherFeedback(FiveDaysForecast weatherData) async {
    ...
    final response = await model.generateContent([
      Content.text(
          "without trying to decorate the text and Shortly, can you tell me what to expect and be prepare for when going out if weather codition is: ${weatherData.current!.condition!.text} and temperature is ${weatherData.current!.tempC} degrees celcius"),
